document.addEventListener('DOMContentLoaded', function() {
    ...
    function addRow() {
      ...
      // Ajouter l'Ã©vÃ©nement pour supprimer la ligne
      row.querySelector('.removeRow').addEventListener('click', function() {
        row.remove();
        calculate();
      });
      
      // Mettre Ã  jour le total de la ligne lors d'un changement
    ...
    function fillAutomatically() {
      ...
      // Ajouter 3 lignes alÃ©atoires
      for(let i = 0; i < 3; i++) {
        addRow();
        const row = document.querySelector('#tableBody tr:last-child');
        const descriptions = ['Ordinateur portable', 'Smartphone', 'Tablette', 'Ecran', 'Clavier'];
    ...
    // Initialiser avec une ligne par dÃ©faut
    addRow();
  });